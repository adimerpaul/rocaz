...
</table><hr><div class='titulo'>DETALLE</div>`;t.forEach(w=>{m+=`<div style='font-size: 12px'><b> ${w.nombre} </b></div>`,w.visible===!0||w.visible===1?m+=`<div><span style='font-size: 18px;font-weight: bold'>${w.cantidadVenta}</span> ${parseFloat(w.precioVenta).toFixed(2)} 0.00
                    <span style='float:right'>${parseFloat(w.precioVenta*w.cantidadVenta).toFixed(2)}</span></div>`:m+=`<div><span style='font-size: 18px;font-weight: bold'>${w.cantidadVenta}</span></div>`}),m+=`<hr>
